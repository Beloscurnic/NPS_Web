    ...
    public partial class GroupQuestionnaire_Get
    {
        public class Handler : IRequestHandler<Query, View_Model_GroupQuestionnaire>
        {
            ...
            public async Task<View_Model_GroupQuestionnaire> Handle(Query request, CancellationToken cancellationToken)
            {
                try
                {
                    var url = authURLs.Get_GroupQuestionnaire(request.LincenseID, request.BaseQueryModel.Token);
                    ...
                    var jsonObj = JsonConvert.DeserializeObject<View_Model_GroupQuestionnaire>(queryResponse);
                    ...
                    if (jsonObj.ErrorCode == 143)
                    {
                        ...
                        return await Handle(new Query { LincenseID= request.LincenseID, BaseQueryModel= request.BaseQueryModel }, cancellationToken);
                    }
                    return jsonObj;
                }
                catch (Exception ex)
                {
                    View_Model_GroupQuestionnaire baseResponse = new View_Model_GroupQuestionnaire()
                    {
                        ErrorCode = 143,
