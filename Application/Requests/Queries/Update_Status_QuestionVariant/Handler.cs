...
namespace Application.Requests.Queries.Update_Status_QuestionVariant
{
    ...
    public class Handler : IRequestHandler<Query, View_Model>
    {
        ...
        public async Task<View_Model> Handle(Query request, CancellationToken cancellationToken)
        {
            try
            {
                var url = authURLs.UpdateStatusQuestionVariant(request.QuestionVariant_ID, request.Oprostnik_ID, request.BaseQueryModel.Token);
                ...
                if (jsonObj.ErrorCode == 143)
                {
                    ...
                    {
                        QuestionVariant_ID = request.QuestionVariant_ID,
